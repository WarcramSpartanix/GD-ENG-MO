...
Texture::Texture(const wchar_t* fullPath) : Resource(fullPath)
{
	...
	HRESULT res = DirectX::LoadFromWICFile(fullPath, DirectX::WIC_FLAGS_NONE, nullptr, image_data);
	...
	if (SUCCEEDED(res))
	{
		...
		GraphicsEngine::getInstance()->getRenderSystem()->m_d3d_device->CreateShaderResourceView(m_texture, &desc, &m_shader_res_view);
	}
	else
	{
